...
const App = () => {
  ...
  const createProject = async (templateId?: string, customName?: string) => {
      ...
      const newProj: Project = { id: '', name: finalName, nodes: initialNodes as any, edges: initialEdges as any, updatedAt: new Date(), ownerId: user.id };
      try {
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${isDark ? 'dark' : ''}`}>
      ...
      <aside className={`bg-slate-900 text-white flex flex-col justify-between transition-all duration-300 w-20 ${isSidebarCollapsed ? 'lg:w-20' : 'lg:w-64'}`}>
        <div>
