        ...
        <div className={`min-h-screen font-sans ${bgPage} transition-colors duration-300 relative overflow-x-hidden`}>
            ...
            <nav className={`border-b sticky top-0 z-30 backdrop-blur-xl ${isDark ? 'border-slate-800 bg-slate-900/80' : 'border-slate-200 bg-white/80'}`}>
                <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
                    <button onClick={onBack} className={`flex items-center gap-2 text-sm font-bold transition-colors py-2 pr-4 rounded-lg ${isDark ? 'text-slate-400 hover:text-white hover:bg-slate-800' : 'text-slate-600 hover:text-indigo-600 hover:bg-slate-100'}`} title="Voltar" aria-label="Voltar">
            ...
            <div className="max-w-6xl mx-auto px-6 py-10 relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                    ...
                    <button
                        ...
                        aria-label="Sugerir nova funcionalidade"
                    >
                ...
                {filteredFeedbacks.length === 0 ? (
                    <div className={`text-center py-24 rounded-3xl border border-dashed flex flex-col items-center justify-center ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-slate-300'}`}>
                        ...
                        <button onClick={() => { setSearchQuery(''); setFilterStatus('ALL'); setFilterType('ALL'); }} className="text-indigo-600 text-sm font-bold hover:underline" title="Limpar todos os filtros" aria-label="Limpar todos os filtros">Limpar Filtros</button>
                    </div>
                ) : (
                        ...
                        {filteredFeedbacks.map((item) => {
                            ...
                            `}
                                >
                                    ...
                                    `}
                                            title={hasVoted ? 'Remover voto' : 'Votar nesta sugestão'}
                                            aria-label={hasVoted ? 'Remover voto' : 'Votar nesta sugestão'}
                                        >
                                            <ChevronUp size={14} className={hasVoted ? 'stroke-[3px]' : ''} aria-hidden="true" />
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fade-in-up">
                    ...
                    >
                        ...
                        <div className={`p-6 border-b flex justify-between items-start flex-shrink-0 ${isDark ? 'border-slate-800 bg-slate-900' : 'border-slate-100 bg-white'}`}>
                            ...
                            <button onClick={() => setSelectedFeedback(null)} className={`absolute top-4 right-4 p-2 rounded-full transition-colors ${isDark ? 'text-slate-400 hover:bg-slate-800 hover:text-white' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`} title="Fechar modal" aria-label="Fechar modal">
                        ...
                        <div className={`flex-1 overflow-y-auto p-8 scrollbar-thin ${isDark ? 'scrollbar-thumb-slate-700' : 'scrollbar-thumb-slate-300'}`}>
                            ...
                            <div className="flex items-center justify-between gap-4 mb-10 pb-10 border-b border-dashed border-slate-200 dark:border-slate-800">
                                ...
                                {(() => {
                                    ...
                                    `}
                                            title={hasVoted ? 'Remover apoio' : 'Apoiar esta ideia'}
                                            aria-label={hasVoted ? 'Remover apoio' : 'Apoiar esta ideia'}
                                        >
                                            {hasVoted ? <CheckCircle size={20} aria-hidden="true" /> : <ThumbsUp size={20} aria-hidden="true" />}
                        ...
                        <div className="p-8 text-center">
                            ...
                            <div className="flex flex-col gap-3">
                                <button
                                    ...
                                    aria-label="Fazer Login / Cadastrar"
                                >
                                    <LogIn size={20} aria-hidden="true" /> Fazer Login / Cadastrar
                                </button>
                                <button
                ...
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                    <div className={`w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up flex flex-col ${isDark ? 'bg-slate-900 border border-slate-700' : 'bg-white'}`}>
                        <div className={`px-8 py-6 border-b flex justify-between items-center ${isDark ? 'border-slate-800 bg-slate-900' : 'border-slate-100 bg-slate-50'}`}>
                            ...
                            <button onClick={() => setIsSubmitModalOpen(false)} className={`p-2 rounded-full transition-colors ${isDark ? 'text-slate-400 hover:bg-slate-800 hover:text-white' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-200'}`} title="Fechar modal" aria-label="Fechar modal">
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            ...
                            <div className="pt-4 flex justify-end gap-3">
                                <button
                                    ...
                                    title="Cancelar nova sugestão"
                                    aria-label="Cancelar nova sugestão"
                                >
                                    Cancelar
                                </button>
                                <button
                                ...
                                >
                                    Enviar Sugestão <Send size={16} aria-hidden="true" />
                                </button>
                            </div>
                        </form>
                    </div>
