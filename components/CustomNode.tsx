...
const CustomNode = ({ id, data, selected }: NodeProps<FunnelNodeData>) => {
  ...
  const renderHandles = () => {
    ...
    // Hit Area is now w-8 h-8 (32px) which is much larger than the visible dot
    // Added pointer-events-auto to ensure it captures mouse events
    const baseClasses = `absolute w-8 h-8 !bg-transparent !border-none z-[100] flex items-center justify-center ${interact}`;
    ...
    // CHANGED: Opacity is now controlled by hover/select state AND Swap Menu State
    const VisualDot = () => (
        <div className={`
    ...
    return (
        <>
            <Handle type="source" position={Position.Top} id="top" className={`${baseClasses} -top-4 left-1/2 -translate-x-1/2`} onClick={(e) => handleConnect(e, 'top')}><VisualDot /></Handle>
            <Handle type="source" position={Position.Right} id="right" className={`${baseClasses} -right-4 top-1/2 -translate-y-1/2`} onClick={(e) => handleConnect(e, 'right')}><VisualDot /></Handle>
            <Handle type="source" position={Position.Bottom} id="bottom" className={`${baseClasses} -bottom-4 left-1/2 -translate-x-1/2`} onClick={(e) => handleConnect(e, 'bottom')}><VisualDot /></Handle>
            <Handle type="source" position={Position.Left} id="left" className={`${baseClasses} -left-4 top-1/2 -translate-y-1/2`} onClick={(e) => handleConnect(e, 'left')}><VisualDot /></Handle>
        </>
    );
  };
            ...
            {showSwapMenu && (
                ...
                >
                    <div className="bg-slate-50 dark:bg-slate-800 p-2 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                        ...
                        <button onClick={() => setShowSwapMenu(false)} className="text-slate-400 hover:text-slate-600"><X size={12}/></button>
                    </div>
              ...
              <div className={containerClass}>
                  ...
                  `}>
                      ...
                      <textarea 
                        ref={noteTextAreaRef}
                        className={`
                            nodrag w-full bg-transparent resize-none outline-none p-3 text-xs font-medium text-slate-700 placeholder-slate-400 leading-relaxed overflow-hidden block box-border
                            ${isPresentation ? 'pointer-events-none' : ''}
                        `}
                        style={{
                            // Gray lines for text, spaced out to match standard lined paper
                            backgroundImage: 'repeating-linear-gradient(transparent, transparent 23px, #e2e8f0 24px)', 
                            lineHeight: '24px',
                            paddingTop: '2px', // Align text with the first line
                            minHeight: '96px'
                        }}
                        value={noteBody}
