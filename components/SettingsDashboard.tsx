...
const SettingsDashboard = ({ user, onUpdateUser, isDark, toggleTheme, lang, setLang, t, projectsCount = 0 }: SettingsDashboardProps) => {
  ...
  const inputClass = `w-full p-3 rounded-xl border outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-medium ${
    isDark ? 'bg-slate-900 border-slate-700 text-white placeholder-slate-600' : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400'
  }`;
                     ...
                     <div className="space-y-6">
                        <div className={`p-8 rounded-2xl border shadow-sm ${cardClass}`}>
                            ...
                            <form className="space-y-4 max-w-md">
                                <div>
                                    <label className={labelClass}>{t('currentPwd')}</label>
                                ...
                                <div className="pt-2">
                                    <button type="button" className="px-6 py-2.5 bg-slate-900 hover:bg-slate-800 dark:bg-slate-700 dark:hover:bg-slate-600 text-white rounded-xl font-bold transition-all shadow-md">
                                        {t('updatePwd')}
                                    </button>
                                </div>
                            </form>
