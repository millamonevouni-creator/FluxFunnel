...
type AuthView = 'LOGIN' | 'FORGOT_PASSWORD' | 'RESET_SENT';

const AuthPage = ({ onAuthSuccess, onBack, t, lang, customSubtitle }: AuthPageProps) => {
        ...
        {isLoading && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 flex items-center justify-center flex-col">
                ...
                <p className="text-sm font-bold text-slate-600 animate-pulse">
                    {currentView === 'FORGOT_PASSWORD' ? 'Enviando link...' : (isLogin ? 'Autenticando...' : 'Criando conta...')}
                </p>
            </div>
        )}
