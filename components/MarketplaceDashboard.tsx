...
import { ShoppingBag, Search, Star, Download, Crown, Lock, Layout, User, Trash2, AlertCircle, TrendingUp, Sparkles, CheckCircle, Eye, ShieldAlert, X, Edit, Save as SaveIcon } from 'lucide-react';
...
const MarketplaceDashboard = ({ userPlan, onDownload, isDark, t, userId }: MarketplaceDashboardProps) => {
    ...
    const fetchMarketplace = async () => {
        setIsLoading(true);
        try {
            const publicTemplates = await api.templates.listPublic(userId);
    ...
    const handleDelete = async (id: string) => {
        if (window.confirm("Tem certeza que deseja excluir esta estratégia pendente?")) {
    ...
    const saveEdit = async () => {
        ...
        await api.templates.update(editingTemplate.id, {
            custom_label: editForm.label,
            custom_description: editForm.description
        });
        ...
        <div className={`flex-1 overflow-y-auto h-full p-8 transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
                            ...
                            <div key={tpl.id} className={`group relative flex flex-col rounded-[2.5rem] border shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden ${cardBg} hover:-translate-y-2`}>
                                ...
                                {tpl.status === 'PENDING' && (
                                    <div className="absolute top-6 right-6 z-10 flex gap-2">
                                        <div className="bg-amber-100 text-amber-700 border border-amber-200 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-sm">
                                            <AlertCircle size={12} /> Em Análise
                                        </div>
                                        {userId === tpl.authorId && ( // Use authorId alias for owner_id
                                            <>
                                                <button onClick={() => startEdit(tpl)} className="bg-white p-1.5 rounded-full text-slate-500 hover:text-indigo-600 shadow-sm border transition-colors" title="Editar">
                                                    <Edit size={14} />
                                                </button>
                                                <button onClick={() => handleDelete(tpl.id)} className="bg-white p-1.5 rounded-full text-slate-500 hover:text-red-600 shadow-sm border transition-colors" title="Excluir">
                                                    <Trash2 size={14} />
                                                </button>
                                            </>
                                        )}
            ...
            {quickLookTemplate && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 bg-black/90 backdrop-blur-xl animate-fade-in-up">
                    <div className={`w-full max-w-6xl rounded-[3rem] border overflow-hidden flex flex-col h-[90vh] ${cardBg}`}>
                        ...
                        <div className="flex-1 bg-slate-50 dark:bg-slate-900 overflow-hidden relative">
                            ...
                            >
                                <Background color={isDark ? "#334155" : "#cbd5e1"} gap={24} />
                    ...
                    <div className={`w-full max-w-lg rounded-2xl shadow-2xl p-8 border ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-3 pt-4">
                                <button onClick={saveEdit} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-500">Salvar Alterações</button>
                                <button onClick={() => setEditingTemplate(null)} className="flex-1 py-3 bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl font-bold">Cancelar</button>
                            </div>
                        </div>
                    </div>
                </div>
