...
import { Save, Moon, Sun, Link as LinkIcon, BookmarkPlus, Undo, Redo, X, Layout, FileText, Lock, Type, Share2, Copy, Check } from 'lucide-react';
...
const FlowCanvas = ({ 
    ...
    t, userPlan, showAIAssistant, onToggleAIAssistant, isSharedView
}: FlowCanvasProps) => {
  ...
  const [saveStep, setSaveStep] = useState<'OPTIONS' | 'TEMPLATE_NAME'>('OPTIONS');
              ...
              <div className={`w-full max-w-lg rounded-2xl shadow-2xl p-8 border ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
                  ...
                  {saveStep === 'OPTIONS' ? (
                      <div className="grid grid-cols-2 gap-4">
                          <button onClick={() => { onSaveProject(nodes, edges); setShowSaveOptions(false); alert("Salvo!"); }} className="flex flex-col items-center p-6 rounded-xl border-2 hover:border-indigo-500 transition-all"><FileText size={32} className="mb-2 text-indigo-600"/> <span className="font-bold">Projeto</span></button>
                          <button onClick={() => { if(userPlan==='FREE') setShowUpgradeModal(true); else {setTemplateNameInput(''); setSaveStep('TEMPLATE_NAME');}}} className="flex flex-col items-center p-6 rounded-xl border-2 hover:border-purple-500 transition-all"><Layout size={32} className="mb-2 text-purple-600"/> <span className="font-bold">Modelo</span></button>
                      </div>
                  ) : (
                      <div className="flex flex-col gap-4">
                          ...
                          <button onClick={() => { onSaveTemplate?.(nodes, edges, templateNameInput); setShowSaveOptions(false); }} className="py-3 bg-indigo-600 text-white rounded-xl font-bold">Salvar Modelo</button>
                      </div>
                  )}
              </div>
          </div>
